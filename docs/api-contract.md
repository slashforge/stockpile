...
AssetMarket = { usdPrice: number | null; priceChange24hPct: number | null; change24hSource: "snapshot" | "jupiter" | null; liquidityUsd: number | null;
  ...
  // Equity.US.<TICKER>/USD (real stock; asOf = publish time, stale outside market hours) when the server has the PythApiKey secret; "jupiter-stock"
...
ChartReason = "unconfigured" | "unresolved" | "unavailable" | "not_tradable" | "insufficient_data"
  // unconfigured = no TokensApiKey secret on the server; unresolved = tokens.xyz does not know the mint; unavailable = provider error/timeout;
