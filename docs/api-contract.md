...
| GET | `/bags/{id}/chart?range=1D` | `getBagChart` | no | `BagChartResponse` (tokens.xyz bag index); `range` = `1D`\|`1W`\|`1M`\|`ALL` (default `1D`, other values => 400); unknown bag => 404; never 500 for provider problems (200 + `reason`) |
...
ChartRange = "1D" | "1W" | "1M" | "ALL"            // riven-cash period mapping: 1D -> 15m candles over 24h, 1W -> 1H over 7d, 1M -> 4H over 30d, ALL -> 1D candles over 2 years
...
ChartLeg = { mint: string | null; symbol: string; weight: number; weightBps: number; change: { pct: number } | null; ok: boolean; reason: ChartReason | null }
  ...
  // points [], change null and a top-level reason. Cached 60s per bag+range.
