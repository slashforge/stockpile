...
export default function SellTokensReviewScreen() {
  ...
  const sell = () => {
    ...
    router.push("/sell-tokens/progress");
    ...
    <View style={styles.root}>
      <ScrollView contentContainerStyle={[styles.padded, styles.content]} showsVerticalScrollIndicator={false}>
        ...
        <Card padded={false}>
          {ready.transactions.map((tx, index) => (
            <View key={`${preparedAt}-${index}`}>
              ...
              <LegRow tx={tx} bag={subject} side="sell" />
            </View>
          ))}
          <Divider />
        ...
        <T variant="caption" tone="tertiary" align="center">
          Only tokens outside your bags are sold, so your bags stay as they are. Swiping signs every
          swap at once. Stockpile covers network fees. This can’t be undone.
        </T>
      </ScrollView>
      <View style={[styles.footer, { paddingBottom: insets.bottom + 8 }]}>
        {expired || prepare.isPending ? (
          ...
          <SlideToConfirm
            ...
            resetKey={slideReset}
            accessibilityHint={`Swipe right to sell ${portionBps / 100}% of ${subject.title}`}
          />
        )}
      </View>
    </View>
