...
import { SELL_PORTIONS, sellTotalOut } from "@/components/stockpile/buy/sell-flow-context";
...
import { useTokenSellFlow } from "@/components/stockpile/sell-tokens/flow-context";
...
export default function SellTokensAmountScreen() {
  ...
  if (mints.length === 0) {
    return (
      <View style={[styles.root, bottom]}>
        <MessageState icon="layers-outline" title="Nothing picked" body="Pick tokens from your holdings to sell them." />
      </View>
    );
  }
  if (!auth.authenticated || !auth.walletAddress) {
    return (
      <View style={[styles.root, bottom]}>
        <MessageState icon="person-circle" title="Sign in to continue" body="Sign in to sell tokens from your wallet." />
      </View>
    );
  }
  ...
  const review = () => {
    flow.runPrepare(flow.request, {
      onDone: (data) => {
        if (data.status === "ready" && data.transactions.length > 0) router.push("/sell-tokens/review");
      },
    });
  };
      ...
      <View style={styles.legsHeader}>
        ...
        <View style={styles.legsMeta}>
          ...
          <T variant="footnote" tone="tertiary">
            {updating || firstLoad ? "Getting prices" : `${mints.length} ${mints.length === 1 ? "token" : "tokens"}`}
          </T>
        </View>
      </View>
