...
const reference = { markPrice: 42.5, tokenPrice: 42.5, impliedValuation: 300_000_000_000, asOf: "2026-09-01" };

describe("pre-IPO detection", () => {
  test("public equities are not pre-IPO", () => {
    expect(isPreIpoAsset(asset())).toBe(false);
    expect(isPreIpoBag({ issuer: "xstocks", assetClass: "public-equity", assets: [asset()] })).toBe(false);
  });
  ...
  test("detects by asset class, issuer, or bag level", () => {
    ...
    expect(isPreIpoBag({ issuer: "prestocks", assetClass: "pre-ipo", assets: [] })).toBe(true);
    expect(
      isPreIpoBag({ issuer: "xstocks", assetClass: "public-equity", assets: [asset(), asset({ issuer: "prestocks" })] }),
    ).toBe(true);
  });
});
...
describe("issuerMarkLabel", () => {
  test("no reference or non-positive marks show nothing", () => {
    ...
    expect(issuerMarkLabel(asset({ reference: { ...reference, markPrice: 0 } }))).toBeNull();
    expect(issuerMarkLabel(asset({ reference: { ...reference, markPrice: Number.NaN } }))).toBeNull();
  });

  test("formats the issuer mark", () => {
    expect(issuerMarkLabel(asset({ reference }))).toBe("Issuer mark $42.50");
    expect(issuerMarkLabel(asset({ reference: { ...reference, markPrice: 1250 } }))).toBe("Issuer mark $1,250");
  });
});
